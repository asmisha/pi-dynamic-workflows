  ...
  return defineTool({
    ...
    get promptGuidelines() {
      return [
        ...
        "Globals: agent(prompt, opts), parallel(thunks), pipeline(items, ...stages), phase(title), bash(cmd, {cwd?, timeoutMs?}), log(msg), args, cwd, budget, workflow('saved-name', args) (nests one level; global caps hold). Helpers built on agent(): verify(item,{reviewers,lens}), judgePanel(attempts,{judges,rubric}), loopUntilDry({round,key}), completenessCheck(args,results), retry(thunk,{attempts,until}), gate(thunk,validator,{attempts}), checkpoint(prompt,opts).",
        "bash(cmd) runs a shell command and returns {stdout, stderr, exitCode, truncated} — check exitCode before trusting stdout. Use it for mechanical steps (grep/build/test) and pipe its output into an agent() prompt for analysis; results are journaled so resume replays them without re-running.",
        ...
        "Subagents have NO parent context: each prompt must carry the task, relevant paths, and the expected output. For machine-readable output pass a plain JSON Schema via opts.schema (not TypeScript/TypeBox). opts.cwd runs an agent in another directory; opts.sessionFile forks an existing Pi session file (JSONL) so the agent starts with that conversation's context (the source file is never mutated — use only when the user asks to inherit a session). With multiple phases, call phase('Exact Title') before each phase's work so agents group correctly. End with a synthesis agent when combining results; return a compact JSON-serializable value.",
