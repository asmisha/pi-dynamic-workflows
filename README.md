...
The object API resolves `PI_CODING_AGENT_DIR` or `~/.pi/agent`, reads `auth.json`, `models.json`, and effective settings through public `@earendil-works/pi-coding-agent` APIs, composes configured provider extensions, and creates one `ModelRuntime` plus one shared `ModelRegistry` per run. SDK catalog refreshes do not use the network during setup. The same runtime is passed to every subagent session in that run, while each session keeps isolated extension state. If `mainModel` is omitted, the configured default provider/model is used when both are present; tiers come from `~/.pi/workflows/model-tiers.json`. `mainModel`, a script's explicit `agent(..., { model })`, and fallback entries use unambiguous `provider/modelId` specs. Existing phase/meta routes, agent-type definitions, and tier settings may also use a bare model id; the shared registry resolves it using the same available-first policy as `WorkflowAgent`. Every selected route, including configured routes, must exist and be authenticated. An unavailable route fails before that agent starts; only explicit `fallbackModel` or `fallbacks` permits a different model (optional entries may be skipped as described below).
...
- **Fan-out orchestration** — `agent()`, `parallel()`, `pipeline()`, `phase()` in a sandboxed script. Total subagents are not capped, and intermediate results stay in variables rather than the chat.
...
| `parallel(thunks)` | Run `() => agent(...)` thunks in parallel; results keep input order on success and ordinary branch errors reject the fan-out. For best effort, catch inside each branch, not on `parallel(...)`, because sibling branches can still be running when the aggregate rejects. |
...
| `readOnly` | Set `true` for reviewers/searchers. Without a named tool policy, it keeps the fixed repository read/search set and, on macOS, a sandboxed `bash` for Git inspection. A resolved `agentType` can retain explicitly named non-mutating extension tools, while repository mutators, unrestricted shell, and workflow controls stay blocked. Sandboxed shell writes are limited to shared `/tmp`, the host OS temp directory (`os.tmpdir()`, typically `/var/folders/.../T` on macOS), private `$TMPDIR`, and `/dev/null`, including checkouts in those temporary areas; paths outside those areas, including ordinary `$HOME` paths, are not writable. Private `$TMPDIR` is deleted when the agent finishes. Put research artifacts needed by the host or other agents under `/tmp` or in a parent-created OS temp directory: they survive agent cleanup but remain OS-temporary. Unsupported platforms omit `bash` rather than falling back to unrestricted execution. It defaults to one automatic retry unless `retries` overrides it. |
...
Use `sessionPath` when a reviewer/worker should continue the same context across runs; use `forkFrom` when it should start from an existing Pi conversation. Persistent-session writers are serialized across runs and processes, so a second writer waits until the current AgentSession finishes cleanup. Workflow subagents bind extensions headlessly, except observational memory (identified by its `om:status` and `om:view` commands). Its background model calls and compaction hook are disabled for fresh, continued, and forked subagent sessions; Pi's native compaction and other extensions remain available. Parent sessions and global settings are unchanged.

Before binding extensions, the runner sets the session-local `pi-dynamic-workflows-subagent` flag to `true`. Extensions that need to recognize this context must register that boolean flag and read it with `pi.getFlag()` in their lifecycle hooks (not at factory load time). It applies to fresh, forked, and continued subagents, including custom transcript paths and agents allowed to delegate; normal sessions in the same process are not marked.
...
Fallback is deliberately narrower than retry: `fallbackModel` and `fallbacks` activate only for model availability/authentication failures, provider usage limits, and gateway/overload failures after SDK retries. Findings, disagreements, ordinary task errors, and schema noncompliance do not select another model. A mid-task handoff switches the existing subagent session rather than launching another agent, preserving the transcript, completed tool work, and cumulative usage. An exhausted chain surfaces the last failure.

`fallbackModel` retains its existing reasoning behavior: explicit `thinking` applies to the backup too; a tier's effort does not carry to another model. With `fallbacks`, each entry's required `thinking` overrides the primary's effort and is mapped by that model's SDK capability. Availability is checked before starting: optional entries are skipped if missing or unauthenticated; any required unavailable entry fails the call. Available entries are attempted in order as eligible failures occur. Changing an entry's model, effort, optional flag, or position invalidates the call's resume cache.

```js
await agent('Inspect the change', {
  model: 'anthropic/claude-opus-5', thinking: 'xhigh', readOnly: true,
  fallbacks: [
    { model: 'google-vertex/gemini-3.8-flash', thinking: 'high', optional: true },
