...
| `readOnly` | Set `true` for reviewers/searchers. Without a named tool policy, it keeps the fixed repository read/search set and, on macOS, a sandboxed `bash` for Git inspection. A resolved `agentType` can retain explicitly named non-mutating extension tools, while repository mutators, unrestricted shell, and workflow controls stay blocked. Sandboxed shell writes are limited to shared `/tmp` (including a checkout located there), private `$TMPDIR`, and `/dev/null`; `$HOME` and paths outside those temporary areas are not writable. `$TMPDIR` is deleted when the agent finishes. Put full research artifacts needed by the host or other agents under `/tmp`: they survive agent cleanup but remain OS-temporary. Unsupported platforms omit `bash` rather than falling back to unrestricted execution. It defaults to one automatic retry unless `retries` overrides it. |
